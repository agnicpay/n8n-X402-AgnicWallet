    ...
    for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
      try {
        ...
        if (authentication === 'oAuth2') {
          ...
          const credentials = await this.getCredentials('agnicWalletOAuth2Api', itemIndex) as any;
        ...
        const headersConfig = this.getNodeParameter('headers', itemIndex, {}) as any;
...
async function makeX402Request(context: IExecuteFunctions, config: any): Promise<any> {
